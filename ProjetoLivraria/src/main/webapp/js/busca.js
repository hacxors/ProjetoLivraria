...

{
	...
	"acessorios": ["FONE","CABO USB", "CARRREGADOR", "LED", "MOUSE"],
	...
	for(var i=0; i < produtos.length ; i++){
		...
		for(var v=0; v < produtos[i].games.length; v++){ 
			if(produtos[i].games[v] == nome){
				window.alert("Temos "+ titulo.value +" em nosso estoque de jogos"); enc++;
			}
		}
	
		for(var v=0; v < produtos[i].livros.length; v++){ 
			if(produtos[i].livros[v] == nome){
				window.alert("Temos "+ titulo.value +" em nosso estoque de livros"); enc++;
			}
		}
	
		for(var v=0; v < produtos[i].filmes.length; v++){ 
			if(produtos[i].filmes[v] == nome){
				window.alert("Temos "+ titulo.value +" em nosso estoque de filmes"); enc++;
			}
		}
	
		for(var v=0; v < produtos[i].musicas.length; v++){ 
			if(produtos[i].musicas[v] == nome){
				window.alert("Temos "+ titulo.value +" em nosso estoque de músicas"); enc++;
			}
		}
		
		for(var v=0; v < produtos[i].acessorios.length; v++){ 
			if(produtos[i].acessorios[v] == nome){
				window.alert("Temos "+ titulo.value +" em nosso estoque de acessórios"); enc++;
			}
		}
	...
	if(titulo.value == ""){
		...
		window.alert("Produtos na categoria FILMES: "+produtos[0].filmes); 
	...
	}else if(nome == "ACESSORIOS" || nome == "ACESSORIOS"){
		window.alert("Produtos nessa categoria ACESSORIOS: "+produtos[0].acessorios);
